...
-   `Equipo1/scripts/jobs`\`: Contienen los jobs que hicimos para nuestro RNA-seq:
    ...
    -   `Equipo1/scripts/jobs/download_data.sge`\`: Este job es el que usamos para descargar nuestros datos
        ...
        -   wget: Es una función que sirve para descargar cosas de la web. <br>
...
En esta fase del analisis nos indica que podria haber error pero es porqe posiblemente se deba a contaminación, lo cual es común en muestras provenientes de piel, especialmente si el individuo estaba enfermo. Aunque sería posible identificar qué bacterias o microorganismos están presentes en las muestras, no es estrictamente necesario en este caso, ya que durante el indexado contra el genoma de referencia, las lecturas correspondientes a microbios no alinearán y, por lo tanto, serán descartadas.

![](images/clipboard-263100148.png) Esta fase evalúa si en las secuencias de las muestras se encuentran nucleótidos no identificados, representados por la letra "N". Es decir, analiza la presencia de bases que el secuenciador no pudo determinar con certeza. En nuestro caso, podemos observar que ninguna de las muestras contiene "N" en sus secuencias, lo que indica una buena calidad en este aspecto.
...
Una vez procesadas las muestras, se observa una reducción en el número de secuencias duplicadas. Esta disminución es un buen indicador, ya que sugiere una mayor diversidad en las lecturas y una menor redundancia, lo cual mejora la calidad del análisis y la representatividad del conjunto de datos.

![](images/clipboard-15205712.png)

En la sección de Sequence Quality Histograms (histogramas de calidad por secuencia), todas las muestras presentaron resultados correctos. Las distribuciones muestran que la mayoría de las lecturas tienen una calidad promedio alta, lo cual indica que las secuencias son confiables y que el proceso de secuenciación fue exitoso. Este resultado es favorable, ya que una buena calidad global de las lecturas es fundamental para obtener análisis precisos en las etapas posteriores.
